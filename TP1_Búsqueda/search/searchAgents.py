...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        space)
        """
        visited_corners = [False] * len(self.corners)
        if self.startingPosition in self.cornerIndices:
            visited_corners[self.cornerIndices[self.startingPosition]] = True
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if next_position in self.cornerIndices:
                next_visited_corners[self.cornerIndices[next_position]] = True
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    remaining_corners = [
        corner for corner, visited in zip(corners, visited_corners) if not visited
    ]
    ...
    for order in itertools.permutations(remaining_corners):
        cost = util.manhattanDistance(position, order[0])
        for i in range(len(order) - 1):
            cost += util.manhattanDistance(order[i], order[i + 1])
